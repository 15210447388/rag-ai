...
public class EmbeddingService {
    ...
    public EmbeddingService(EmbeddingModel embeddingModel, StringRedisTemplate redisTemplate, TokenMetrics tokenMetrics, @Lazy EmbeddingService self) {
    ...
    public List<float[]> embedBatch(List<String> texts) {
        ...
        for (int i = 0; i < texts.size(); i++) {
            ...
            if (cachedStr != null) {
                try {
                    ...
                    log.warn("[Embedding] 缓存数据损坏，回退到 API：key={}, err={}", cacheKey, ex.getMessage());
        ...
        log.debug("[Embedding] 总数={}，缓存命中={}，需要调API={}", texts.size(), cached.size(), missedTexts.size());
    ...
    @Retryable(retryFor = {ResourceAccessException.class,        // 网络 IO 异常（连接超时、读超时、Connection reset）
            ...
            java.util.concurrent.TimeoutException.class}, noRetryFor = HttpClientErrorException.class,   // 4xx 不重试（401/400/413）
    ...
    public List<float[]> embedFromApi(List<String> texts) {
        ...
        for (int start = 0; start < texts.size(); start += BATCH_SIZE) {
            ...
            response.getResults().stream().sorted(Comparator.comparingInt(r -> r.getIndex())).forEach(r -> result.add(r.getOutput()));

            log.debug("[Embedding] 批次{}/{}，size={}，耗时={}ms", start / BATCH_SIZE + 1, (texts.size() + BATCH_SIZE - 1) / BATCH_SIZE, batch.size(), elapsed);
        }

        log.info("[Embedding] API调用完成，共{}条，消耗Token={}", texts.size(), totalTokens.get());
